        ...
        for stage, (embed_dim, num_layer, num_head, mlp_ratio) in enumerate(zip(embed_dims, num_layers, num_heads, mlp_ratios)):
            ...
            blocks += [
                Block(
                    ...
                    attn_drop=attn_drop_rate, drop_path=dpr[sum(num_layers[:stage]) + i], norm_layer=norm_layer, act_layer=self.act_layer, attention=LightViTAttention)
...
@register_model
def lightvit_tiny_cifar(pretrained=False, **kwargs):
    model_kwargs = dict(
        img_size=32,  # CIFAR 이미지 크기에 맞게 수정
        patch_size=6,  # CIFAR 이미지 크기에 맞게 패치 크기를 줄임
        embed_dims=[64, 128, ],  # 임베딩 차원은 그대로 유지하거나 실험에 따라 조정 가능
        num_layers=[2, 2, ],  # CIFAR 데이터셋은 작으므로 레이어 수를 줄임
        num_heads=[2, 4, ],  # 헤드 수는 그대로 유지하거나 실험에 따라 조정 가능
        mlp_ratios=[4, 4, ],  # CIFAR에 맞게 MLP 비율을 조정
        num_tokens=8,  # 글로벌 토큰의 수는 실험에 따라 조정 가능
    )
...
def replace_activations(model, new_act_layer):
    for name, module in model.named_children():
        # 해당 layer가 활성화 함수인지 확인
        if isinstance(module, nn.GELU):
            # 새로운 활성화 함수로 교체
            setattr(model, name, new_act_layer)
        else:
            # 재귀적으로 자식 레이어에 대해서도 확인
            replace_activations(module, new_act_layer)